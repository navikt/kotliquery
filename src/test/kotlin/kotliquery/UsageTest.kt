...
class UsageTest {
    ...
    fun transactionWithNonLabeledReturnGetsCommitted() {
        ...
        using(sessionOf(testDataSource)) { session ->
            val membersCount =
                session.single(queryOf("select count(*) as count from members")) { row ->
                    row.int("count")
                }
    ...
    fun batchPreparedStatement() {
        using(sessionOf(testDataSource)) { session ->
            ...
            assertEquals(
                listOf(Pair(1, nowAtEpoch), Pair(2, nowAtEpoch), Pair(3, nowAtEpoch)),
    ...
    fun batchPreparedStatementReturnGeneratedId() {
        using(sessionOf(testDataSource)) { session ->
            ...
            val now = Instant.now()
            val res =
                session.batchPreparedStatementAndReturnGeneratedKeys(
    ...
    private fun withPreparedStmt(
        query: Query,
        closure: (PreparedStatement) -> Unit,
    ) {
        using(sessionOf(testDataSource)) { session ->
    ...
    private fun String.extractQueryFromPreparedStmt(): String = this.replace(Regex("^.*?: "), "").normalizeSpaces()
}
