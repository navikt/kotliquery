...
open class Session(
    ...
    private fun checkActive() {
        ...
        if (ctx != null && !ctx.active) {
            throw IllegalStateException(
                "This session's transaction has already ended. Do not use sessions after the transaction block completes.",
            )
        }
    }
    ...
    inline fun <A> transaction(operation: (TransactionalSession) -> A): A {
        ...
        try {
            connection.begin()
            transactional = true
            val tx = TransactionalSession(connection, returnGeneratedKeys, autoGeneratedKeys, strict, queryTimeout)
